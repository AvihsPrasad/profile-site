<template>
  <div class="photo_gallery">
    <div class="grid-container">
      <div
        v-for="(data, index) in active"
        :key="index"
        data-aos="fade-up"
        :data-aos-delay="100 * index"
        @click="model('one', data)"
      >
        <img
          class="grid-item"
          :class="'grid-item-' + index"
          v-bind:src="require('../assets/images/insta/' + data)"
          :alt="data"
        />
    ...
    <div id="modal-container" :class="someval" @click="model('out')">
      <div class="modal-background">
        <div class="modal">
          <!-- <button @click="model('out')">out</button>
          <h2>I'm a Modal</h2>
          <p>Hear me roar. {{ imgval }}</p> -->
          <div v-if="imgval">
            <img v-bind:src="require('../assets/images/insta/' + imgval)" />
          </div>
        </div>
      </div>
    </div>
    <!-- <div class="content">
      ...
      <div class="buttons">
        <div id="one" class="button" @click="model('one')">Unfolding</div>
...
export default class PhotoGallery extends Vue {
  ...
  model(data: any, img: any) {
    // alert('asas')
    this.someval = data;
    this.imgval = img; // return data;
  }
}
</script>
...
.photo_gallery {
  ...
  .grid-container {
    ...
    div {
      ...
      width: 100%;
      // border: solid 2px var(--dynamic-title-color);
      padding: 5px;
